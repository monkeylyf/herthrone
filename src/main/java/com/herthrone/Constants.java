package com.herthrone;

/**
 * Created by yifeng on 4/8/16.
 */
public class Constants {

  public static class Hero {
    public static final String ANDUIN_WRYNN = "Anduin Wrynn";
    public static final String GULDAN = "Gul'dan";
    public static final String JAINA_PROUDMOORE = "Jaina Proudmoore";
    public static final String MALFURION_STORMRAGE = "Malfurion Stormrage";
    public static final String REXXAR = "Rexxar";
    public static final String THRALL = "Thrall";
    public static final String UTHER_LIGHTBRINGER = "Uther Lightbringer";
    public static final String VALEERA_SANGUINAR = "Valeera Sanguinar";
    public static final String GARROSH_HELLSCREAM = "Garrosh Hellscream";
  }

  public static class Class {
    public static final String WARRIOR = "Warrior";
    public static final String PRIEST = "Priest";
    public static final String ROGUE = "Rogue";
    public static final String MAGE = "Mage";
    public static final String PALADIN = "Paladin";
    public static final String WARLOCK = "Warlock";
    public static final String SHAMAN = "Shaman";
    public static final String DRUID = "Druid";
    public static final String HUNTER = "Hunter";
  }

  public static class Weapon {
    public static final String FIERY_WAR_AEX = "FieryWarAxe";
    public static final String WICKED_KNIFE = "WickedKnife";
  }

  public static class HeroPower {
    public static final String ARMOR_UP = "ArmorUp";
    public static final String DAGGER_MASTERY = "DaggerMastery";
    public static final String FIRE_BLAST = "FireBlast";
    public static final String LESSER_HEAL = "LesserHeal";
    public static final String LIFE_TAP = "LifeTap";
    public static final String REINFORCE = "Reinforce";
    public static final String SHAPE_SHIFT = "Shapeshift";
    public static final String STEADY_SHOT = "SteadyShot";
    public static final String TOTEMIC_CALL = "TotemicCall";
  }
}
